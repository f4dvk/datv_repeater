...
gst-launch-1.0  -q v4l2src device=/dev/video1 ! "video/x-raw,width=640, height=480, format=(string)YUY2" !  nvvidconv flip-method=0  ! "video/x-raw(memory:NVMM), format=(string)I420, framerate=(fraction)30/1"  ! nvvidconv ! "video/x-raw(memory:NVMM), width=(int)$VIDEO_RESX, height=(int)$VIDEO_RESY, format=(string)I420"  ! omxh264enc   vbv-size=15 control-rate=2 bitrate=$VIDEOBITRATE peak-bitrate=$VIDEOPEAKBITRATE insert-sps-pps=1 insert-vui=1 cabac-entropy-coding=1 preset-level=3 profile=8 iframeinterval=$VIDEO_GOP ! "video/x-h264, level=(string)4.1, stream-format=(string)byte-stream" ! queue ! mux. alsasrc device=plughw:2 ! 'audio/x-raw, format=S16LE, layout=interleaved, rate=48000, channels=1' ! voaacenc bitrate=$AUDIO_BITRATE ! queue  ! mux. mpegtsmux alignment=7 name=mux ! queue ! udpsink host=239.252.252.1 port=10002 | ffmpeg -i udp://@239.252.252.1:10002 -ss 1 -c:v copy -max_delay $PCR_PTS -muxrate $BITRATE_TS -c:a copy -f mpegts -metadata service_provider=$SERVICE_PROVIDER -metadata service_name=$CALL -streamid 0:256 -nostdin $ffmpegoutput
;;
...
gst-launch-1.0 -v v4l2src device=/dev/video1 ! "video/x-raw,width=640, height=480, format=(string)YUY2" !  nvvidconv flip-method=0  ! "video/x-raw(memory:NVMM), format=(string)I420, framerate=(fraction)30/1" ! nvvidconv ! "video/x-raw(memory:NVMM), width=(int)$VIDEO_RESX, height=(int)$VIDEO_RESY, format=(string)I420" ! omxh265enc  control-rate=2 bitrate=$VIDEOBITRATE peak-bitrate=$VIDEOPEAKBITRATE  preset-level=3 iframeinterval=$VIDEO_GOP ! "video/x-h265,stream-format=(string)byte-stream" ! queue ! mux. alsasrc device=plughw:2 ! 'audio/x-raw, format=S16LE, layout=interleaved, rate=48000, channels=1' ! voaacenc bitrate=$AUDIO_BITRATE ! queue  ! mux. mpegtsmux alignment=7 name=mux !queue max-size-time=10000000000 max-size-bytes=0 max-size-buffers=0 ! fdsink | ffmpeg -i - -ss 8 -c:v copy -max_delay $PCR_PTS -muxrate $BITRATE_TS -c:a copy -f mpegts -metadata service_provider=$SERVICE_PROVIDER -metadata service_name=$CALL -streamid 0:256 $ffmpegoutput
;;
*)
echo "Wrong codec" ;;
esac
...
case "$CODEC" in
"H264")
gst-launch-1.0 ximagesrc use-damage=0 ! "video/x-raw,framerate=25/1" ! videoscale method=0 ! "video/x-raw,width=640,height=480" ! nvvidconv flip-method=0 ! "video/x-raw(memory:NVMM), format=(string)I420" ! nvvidconv ! "video/x-raw(memory:NVMM), width=(int)$VIDEO_RESX, height=(int)$VIDEO_RESY, format=(string)I420"  ! omxh264enc   vbv-size=15 control-rate=2 bitrate=$VIDEOBITRATE peak-bitrate=$VIDEOPEAKBITRATE insert-sps-pps=1 insert-vui=1 cabac-entropy-coding=1 preset-level=3 profile=8 iframeinterval=$VIDEO_GOP ! "video/x-h264, level=(string)4.1, stream-format=(string)byte-stream" ! queue ! mux. alsasrc device=plughw:2 ! 'audio/x-raw, format=S16LE, layout=interleaved, rate=48000, channels=1' ! voaacenc bitrate=$AUDIO_BITRATE ! queue  ! mux. mpegtsmux alignment=7 name=mux ! queue ! udpsink host=239.252.252.1 port=10002 | ffmpeg -i udp://@239.252.252.1:10002 -ss 1 -c:v copy -max_delay $PCR_PTS -muxrate $BITRATE_TS -c:a copy -f mpegts -metadata service_provider=$SERVICE_PROVIDER -metadata service_name=$CALL -streamid 0:256 -nostdin $ffmpegoutput
;;
...
case "$CODEC" in
"H264")
gst-launch-1.0 -v videotestsrc ! "video/x-raw,framerate=25/1" ! videoscale method=0 ! "video/x-raw,width=$VIDEO_RESX,height=$VIDEO_RESY" ! nvvidconv flip-method=0 ! "video/x-raw(memory:NVMM), format=(string)I420" ! nvvidconv ! "video/x-raw(memory:NVMM), width=(int)$VIDEO_RESX, height=(int)$VIDEO_RESY, format=(string)I420"  ! omxh264enc   vbv-size=15 control-rate=2 bitrate=$VIDEOBITRATE peak-bitrate=$VIDEOPEAKBITRATE insert-sps-pps=1 insert-vui=1 cabac-entropy-coding=1 preset-level=3 profile=8 iframeinterval=$VIDEO_GOP ! "video/x-h264, level=(string)4.1, stream-format=(string)byte-stream" ! queue ! mux. alsasrc device=plughw:2 ! 'audio/x-raw, format=S16LE, layout=interleaved, rate=48000, channels=1' ! voaacenc bitrate=$AUDIO_BITRATE ! queue  ! mux. mpegtsmux alignment=7 name=mux ! queue ! udpsink host=239.252.252.1 port=10002 | ffmpeg -i udp://@239.252.252.1:10002 -ss 1 -c:v copy -max_delay $PCR_PTS -muxrate $BITRATE_TS -c:a copy -f mpegts -metadata service_provider=$SERVICE_PROVIDER -metadata service_name=$CALL -streamid 0:256 -nostdin $ffmpegoutput
;;
